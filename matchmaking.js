export class MatchmakingQueue {
  ...
  async fetch(request) {
    ...
    try {
      ...
      const { heroes, sessionId } = body;
      if (!heroes || !Array.isArray(heroes) || heroes.length === 0 || !sessionId || typeof sessionId !== 'string') {
        console.log('Invalid heroes or sessionId:', { heroes, sessionId });
        return new Response(
          JSON.stringify({ status: 'error', message: 'Heroes must be a non-empty array and sessionId a string' }),
          { status: 400, headers: corsHeaders }
        );
      }
      console.log('Processing matchmaking for:', { heroes, sessionId });
      ...
      if (!this.players.find(p => p.sessionId === sessionId)) {
        this.players.push({ heroes, id: crypto.randomUUID(), sessionId, timestamp: now });
      ...
      if (this.players.length >= 2) {
        ...
        // Generate game URL with heroes for both players
        const gameId = crypto.randomUUID();
        const gameUrl = `https://oddheroes.online/game.html?player1Heroes=${encodeURIComponent(JSON.stringify(player1.heroes))}&player2Heroes=${encodeURIComponent(JSON.stringify(player2.heroes))}&gameId=${gameId}`;
