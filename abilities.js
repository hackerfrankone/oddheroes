...
// Map of ability choices to their game logic
const abilityEffects = {
  ...
  'offensive': {
    execute: (player) => {
      console.log(`${player.name} uses Offensive: Activates machine gun and missiles, dealing 30 damage.`);
      // Example: player.dealDamage({ type: 'ranged', amount: 30 });
      return { success: true, message: 'Offensive activated' };
    }
  },
  'defensive': {
    execute: (player) => {
      console.log(`${player.name} uses Defensive: Transports allies and heals 20 HP.`);
      // Example: player.transportAllies(); player.healAllies(20);
      return { success: true, message: 'Defensive activated' };
    }
  }
};
